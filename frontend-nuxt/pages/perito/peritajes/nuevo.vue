<template>
  <div>
    <div class="mb-6">
      <h1 class="text-3xl font-bold text-gray-900">Nuevo Peritaje</h1>
      <p class="text-gray-600 mt-2">Crea un nuevo peritaje de revisión precompra</p>
    </div>
            ...
            <div class="space-y-4">
              <div class="space-y-2">
                ...
                <input 
                  ...
                  placeholder="0" 
                  min="0"
                  class="w-full px-4 py-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all duration-200 bg-white shadow-sm text-lg font-semibold text-slate-900" 
                />
              </div>
...
const handleSubmit = async () => {
  ...
  try {
    ...
    Object.keys(form).forEach(key => {
      if (key === 'carroceria' || key === 'chasis' || key === 'llantas') {
        // Campos JSONB
    ...
    if (!response.ok) {
      ...
      throw new Error(error.message || 'Error al crear el peritaje')
    }

    await navigateTo('/perito/peritajes')
  } catch (error) {
